...
void mp_image_params_guess_csp(struct mp_image_params *params)
{
    ...
    if (!mp_trc_is_hdr(params->color.gamma)) {
        ...
        params->color.hdr = mp_hdr_metadata_empty;
    }
...
struct mp_image *mp_image_from_av_frame(struct AVFrame *src)
{
    ...
    mp_map_hdr_metadata(&dst->params.color.hdr,
                        (void *)(mdm ? mdm->data : NULL),
                        (void *)(clm ? clm->data : NULL),
                        (void *)(dhp ? dhp->data : NULL)
                       );
...
struct AVFrame *mp_image_to_av_frame(struct mp_image *src)
{
    ...
    *(struct mp_image_params *)dst->opaque_ref->data = src->params;
    ...
    mp_avframe_set_color(dst, (struct mp_colorspace) {
            .primaries = src->params.color.primaries,
            .gamma = src->params.color.gamma,
